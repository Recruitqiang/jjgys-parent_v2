...
public class JjgFbgcSdgcSdhpServiceImpl extends ServiceImpl<JjgFbgcSdgcSdhpMapper, JjgFbgcSdgcSdhp> implements JjgFbgcSdgcSdhpService {
            ...
            for (Map<String, Object> m : sdmclist)
            {
                for (String k : m.keySet()){
                    ...
                    if (substring.equals("Z") || substring.equals("Y")){
                        DBtoExcelZYsd(proname,htd,fbgc,sdmc,username);
                    }else {
     ...
     */
    private void DBtoExcelsd(String proname, String htd, String fbgc, String sdmc, String username) throws IOException, ParseException {
     ...
     * @throws ParseException
     */
    private void DBtoExcelZYsd(String proname, String htd, String fbgc, String sdmc, String username) throws IOException, ParseException {
        XSSFWorkbook wb = null;
        QueryWrapper<SysUser> sysUserQueryWrapper = new QueryWrapper<>();
        sysUserQueryWrapper.eq("username", username);
        SysUser one = sysUserService.getOne(sysUserQueryWrapper);
    ...
    public List<Map<String, Object>> lookJdbjg(CommonInfoVo commonInfoVo) throws IOException {
        ...
        String htd = commonInfoVo.getHtd();
        //String fbgc = commonInfoVo.getFbgc();
        List<Map<String, Object>> selectsdmc = selectsdmc1(proname, htd);
        ...
        for (int i=0;i<selectsdmc.size();i++){
            ...
            if (!f.exists()) {
                ...
                List<Map<String,Object>> zhlist = jjgFbgcSdgcSdhpMapper.selectzh1(proname,htd,sdmc);
                String zh = zhlist.get(0).get("zh").toString();
                    ...
                    .registerReadListener(
                            new ExcelHandler<JjgFbgcSdgcSdhpVo>(JjgFbgcSdgcSdhpVo.class) {
                                ...
                                public void handle(List<JjgFbgcSdgcSdhpVo> dataList) {
                                    ...
                                    {
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getZh())) {
                                            throw new JjgysException(20001, "第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getSdmc())) {
                                            throw new JjgysException(20001, "第"+rowNumber+"行的数据中，隧道名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getLmlx())) {
                                            throw new JjgysException(20001, "第"+rowNumber+"行的数据中，路面类型为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getWz())) {
                                            throw new JjgysException(20001, "第"+rowNumber+"行的数据中，位置为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(fbgcSdgcSdhpVo.getQsds()) || StringUtils.isEmpty(fbgcSdgcSdhpVo.getQsds())) {
                                            throw new JjgysException(20001, "第"+rowNumber+"行的数据中，前视读数值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(fbgcSdgcSdhpVo.getHsds()) || StringUtils.isEmpty(fbgcSdgcSdhpVo.getHsds())) {
                                            throw new JjgysException(20001, "第"+rowNumber+"行的数据中，后视读数值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(fbgcSdgcSdhpVo.getLength()) || StringUtils.isEmpty(fbgcSdgcSdhpVo.getLength())) {
                                            throw new JjgysException(20001, "第"+rowNumber+"行的数据中，长值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(fbgcSdgcSdhpVo.getSjz()) || StringUtils.isEmpty(fbgcSdgcSdhpVo.getSjz())) {
                                            throw new JjgysException(20001, "第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(fbgcSdgcSdhpVo.getYxps()) || StringUtils.isEmpty(fbgcSdgcSdhpVo.getYxps())) {
                                        ...
                                        fbgcSdgcSdhp.setUsername(commonInfoVo.getUsername());
    ...
    public List<Map<String, Object>> selectsdmc1(String proname, String htd) {
        List<Map<String,Object>> sdmclist = jjgFbgcSdgcSdhpMapper.selectsdmc1(proname,htd);
        return sdmclist;
    }
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        ...
                        map.put("合格点数", hgdsz1);
                        map.put("合格率", hglz1);
                        map.put("设计值", slSheet.getRow(6).getCell(9).getStringCellValue());
                        jgmap.add(map);
                    }
    ...
    public int selectnum(String proname, String htd) {
        int selectnum = jjgFbgcSdgcSdhpMapper.selectnum(proname, htd);
        return selectnum;
    }

    @Override
    public int selectnumname(String proname) {
        int selectnum = jjgFbgcSdgcSdhpMapper.selectnumname(proname);
        return selectnum;

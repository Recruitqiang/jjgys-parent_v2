...
public class JjgFbgcLmgcServiceImpl implements JjgFbgcLmgcService {
    ...
    public void getFile(String workpath, String proname, String htd) {
        ...
        List<File> list=new ArrayList<>();
        list.add(new File(workpath+File.separator+htd+File.separator+"12沥青路面压实度.xlsx"));
        list.add(new File(workpath+File.separator+htd+File.separator+"13路面弯沉(贝克曼梁法).xlsx"));
        list.add(new File(workpath+File.separator+htd+File.separator+"13路面弯沉(落锤法).xlsx"));
        list.add(new File(workpath+File.separator+htd+File.separator+"15沥青路面渗水系数.xlsx"));
        list.add(new File(workpath+File.separator+htd+File.separator+"16混凝土路面强度.xlsx"));
        list.add(new File(workpath+File.separator+htd+File.separator+"17混凝土路面相邻板高差.xlsx"));
        list.add(new File(workpath+File.separator+htd+File.separator+"20构造深度手工铺沙法.xlsx"));
        ...
        if (lmlx.size()>0){
            for (Map<String, String> map : lmlx) {
                String lxlx = map.get("lxlx");
                if(!Objects.equals(lxlx, "主线")){
                    list.add(new File(workpath+File.separator+"24路面横坡-"+lxlx+".xlsx"));
                }

    ...
    public void getFile(String workpath, String proname, String htd) {
        ...
        if (qlmclist.size() > 0) {
            for (Map<String, Object> m : qlmclist) {
                for (String k : m.keySet()) {
                    String qlmc = m.get(k).toString();
                    list.add(new File(workpath +File.separator+htd+File.separator+ "34桥面平整度3米直尺法-" + qlmc + ".xlsx"));
                }
            }
        }
        qlmclist.clear();
        ...
        if (qlmclist.size() > 0) {
            for (Map<String, Object> m : qlmclist) {
                for (String k : m.keySet()) {
                    String qlmc = m.get(k).toString();
                    list.add(new File(workpath +File.separator+htd+File.separator+ "35桥面横坡-" + qlmc + ".xlsx"));
                }
            }
        }
        qlmclist.clear();
        ...
        if (qlmclist.size() > 0) {
            for (Map<String, Object> m : qlmclist) {
                for (String k : m.keySet()) {
                    String qlmc = m.get(k).toString();
                    list.add(new File(workpath +File.separator+htd+File.separator+ "37构造深度手工铺沙法-" + qlmc + ".xlsx"));
                }
            }
        }

        list.add(new File(workpath +File.separator+htd+File.separator+ "29桥梁上部砼强度.xlsx"));
        list.add(new File(workpath +File.separator+htd+File.separator+ "30桥梁上部主要结构尺寸.xlsx"));
        list.add(new File(workpath +File.separator+htd+File.separator+ "31桥梁上部保护层厚度.xlsx"));
        list.add(new File(workpath +File.separator+htd+File.separator+ "25桥梁下部墩台砼强度.xlsx"));

...
public class JjgFbgcSdgcGssdlqlmhdzxfServiceImpl extends ServiceImpl<JjgFbgcSdgcGssdlqlmhdzxfMapper, JjgFbgcSdgcGssdlqlmhdzxf> implements JjgFbgcSdgcGssdlqlmhdzxfService {
        ...
        if (sdmclist.size()>0){
            for (Map<String, Object> m : sdmclist) {
                for (String k : m.keySet()){
                    ...
                    if (!looksdjdb.isEmpty() && looksdjdb != null  ){
    ...
    public int selectnum(String proname, String htd) {
        int selectnum = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectnum(proname, htd);
        return selectnum;
    }

    @Override
    public int selectnumname(String proname) {
        int selectnum = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectnumname(proname);
        return selectnum;
    }
}

...
import glgc.jjgys.model.projectvo.sdgc.JjgFbgcSdgcZdhpzdVo;
import glgc.jjgys.model.system.SysRole;
import glgc.jjgys.model.system.SysUser;
import glgc.jjgys.model.system.SysUserRole;
import glgc.jjgys.system.easyexcel.ExcelHandler;
import glgc.jjgys.system.exception.JjgysException;
import glgc.jjgys.system.mapper.JjgFbgcSdgcZdhpzdMapper;
 ...
 */
@Service
public class JjgFbgcSdgcZdhpzdServiceImpl extends ServiceImpl<JjgFbgcSdgcZdhpzdMapper, JjgFbgcSdgcZdhpzd> implements JjgFbgcSdgcZdhpzdService {

    @Autowired
    private JjgFbgcSdgcZdhpzdMapper jjgFbgcSdgcZdhpzdMapper;
    ...
    public void generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgFbgcSdgcZdhpzdMapper.selectlx(proname,htd);
        for (Map<String, Object> map : lxlist) {
            String zx = map.get("lxbs").toString();
            int num = jjgFbgcSdgcZdhpzdMapper.selectcdnum(proname,htd,zx);
            int cds = 0;
            if (num == 1){
                cds = 2;
            }else {
    ...
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, String username) throws IOException, ParseException {
        ...
        List<Map<String, Object>> sdzxList = new ArrayList<>();
        List<Map<String, Object>> sdyxList = new ArrayList<>();
        if (rolecode.equals("YH")){
            sdzxList = jjgFbgcSdgcZdhpzdMapper.selectzfListyh(proname, htd,zx, result,username);
            sdyxList = jjgFbgcSdgcZdhpzdMapper.selectyfListyh(proname, htd,zx, result,username);
        }else {
            sdzxList = jjgFbgcSdgcZdhpzdMapper.selectzfList(proname, htd,zx, result);
            sdyxList = jjgFbgcSdgcZdhpzdMapper.selectyfList(proname, htd,zx, result);
        }



        List<Map<String, Object>> sdzfList = montageIRI(sdzxList);
        Collections.sort(sdzfList, new Comparator<Map<String, Object>>() {
            @Override
        ...
        List<Map<String, Object>> sdyfList = montageIRI(sdyxList);
        Collections.sort(sdyfList, new Comparator<Map<String, Object>>() {
            @Override
            public int compare(Map<String, Object> o1, Map<String, Object> o2) {
                // 名字相同时按照 qdzh 排序
                Double qdzh1 = Double.parseDouble(o1.get("qdzh").toString());
        ...
        double zdzh = Double.parseDouble(sdzfList.get(0).get("qdzh").toString());
        double finzdzh = Double.parseDouble(sdzfList.get(sdzfList.size() - 1).get("qdzh").toString());
        List<Map<String, Object>> sdzf = decrementNumberByStep(zdzh, finzdzh, sdzfList, cdsl);

        double yzdzh = Double.parseDouble(sdyfList.get(0).get("qdzh").toString());
        double yfinzdzh = Double.parseDouble(sdyfList.get(sdyfList.size() - 1).get("qdzh").toString());
        List<Map<String, Object>> sdyf = decrementNumberByStep(yzdzh, yfinzdzh, sdyfList, cdsl);
        Collections.sort(sdzf, new Comparator<Map<String, Object>>() {
            @Override
            public int compare(Map<String, Object> o1, Map<String, Object> o2) {
                // 名字相同时按照 qdzh 排序
                Double qdzh1 = Double.parseDouble(o1.get("qdzh").toString());
        ...
        Collections.sort(sdyf, new Comparator<Map<String, Object>>() {
            @Override
            public int compare(Map<String, Object> o1, Map<String, Object> o2) {
                // 名字相同时按照 qdzh 排序
                Double qdzh1 = Double.parseDouble(o1.get("qdzh").toString());
        ...
        writeExcelData(proname, htd, sdzf, sdyf,cdsl, sjz,zx);
    ...
    private void writeExcelData(String proname, String htd, List<Map<String, Object>> sdzxList, List<Map<String, Object>> sdyxList, int cdsl, String sjz, String zx) throws IOException, ParseException {
        ...
        String fname="49隧道路面平整度-"+zx+".xlsx";
        ...
        try {
            ...
            if (sdzxList.size() >0 && !sdzxList.isEmpty()){
                ...
                DBtoExcel(proname,htd,mapslist,wb,sheetmame,cdsl,sjz);
            }
    ...
    private void fillTitleCellData(XSSFSheet sheet, int tableNum, String proname, String htd, String name, String sj, String sheetname, int cdsl, String sjz) {
        ...
        sheet.getRow(tableNum * a + 2).getCell(5).setCellValue("隧道工程");
    ...
    private List<Map<String, Object>> lookdata(String proname, String htd, String zx, int cds) throws IOException {
        ...
        File f = new File(filepath + File.separator + proname + File.separator + htd + File.separator + "49隧道路面平整度-"+zx+".xlsx");
        if (!f.exists()) {
            return new ArrayList<>();
        } else {
    ...
    public void exportpzd(HttpServletResponse response, String cdsl) throws IOException {
        ...
        ExcelUtil.writeExcelMultipleSheets(response, null, fileName, sheetName, new JjgFbgcSdgcZdhpzdVo());
        ...
        for (int i = 0; i < number; i++) {
            ...
            try {
                EasyExcel.read(file.getInputStream())
                        .sheet(sheetIndex)
                        .head(JjgFbgcSdgcZdhpzdVo.class)
                        .headRowNumber(1)
                        .registerReadListener(
                                new ExcelHandler<JjgFbgcSdgcZdhpzdVo>(JjgFbgcSdgcZdhpzdVo.class) {
                                    @Override
                                    public void handle(List<JjgFbgcSdgcZdhpzdVo> dataList) {
                                        for(JjgFbgcSdgcZdhpzdVo pzdVo: dataList)
                                        {
                                            JjgFbgcSdgcZdhpzd pzd = new JjgFbgcSdgcZdhpzd();
                                            ...
                                            pzd.setCd(sheetName);
                                            jjgFbgcSdgcZdhpzdMapper.insert(pzd);
                                        }
                                    }
                                }
                        ).doRead();
    ...
    public List<Map<String, Object>> lookJdb(CommonInfoVo commonInfoVo, String value) throws IOException {
        ...
        int cds = jjgFbgcSdgcZdhpzdMapper.selectcdnum(proname,htd,sdmc);

...
public class JjgFbgcQlgcXbSzdServiceImpl extends ServiceImpl<JjgFbgcQlgcXbSzdMapper, JjgFbgcQlgcXbSzd> implements JjgFbgcQlgcXbSzdService {
    ...
    public List<Map<String, Object>> lookJdbjg(CommonInfoVo commonInfoVo) throws IOException {
        ...
        if(!f.exists()){
            ...
            if(proname.equals(xmname.toString()) && title.equals(bt.toString()) && htd.equals(htdname.toString())){
                ...
                List listscz = new ArrayList();
                Set listyxpc = new HashSet();
                for (int i = 5; i<slSheet.getPhysicalNumberOfRows();i++){
                    ...
                    if (!slSheet.getRow(i).getCell(5).getStringCellValue().equals("")){
                        if (!slSheet.getRow(i).getCell(5).getStringCellValue().equals("实测值(mm)") && !slSheet.getRow(i).getCell(5).getStringCellValue().equals("")){
                            listscz.add(slSheet.getRow(i).getCell(5).getStringCellValue());
                        }

                        if (!slSheet.getRow(i).getCell(3).getStringCellValue().equals("允许偏差（mm）") && !slSheet.getRow(i).getCell(3).getStringCellValue().equals("") ){
                            listyxpc.add(StrUtil.trim(slSheet.getRow(i).getCell(3).getStringCellValue()));
                        }
                    }
                }
                ...
                jgmap.put("scz",listscz);
                jgmap.put("yxpc",listyxpc);
                mapList.add(jgmap);
                return mapList;
            }else {
                return null;
                            ...
                            new ExcelHandler<JjgFbgcQlgcXbSzdVo>(JjgFbgcQlgcXbSzdVo.class) {
                                ...
                                public void handle(List<JjgFbgcQlgcXbSzdVo> dataList) {
                                    ...
                                    for(JjgFbgcQlgcXbSzdVo xbSzdVo: dataList)
                                    {
                                        ...
                                        fbgcQlgcXbSzd.setUsername(commonInfoVo.getUsername());
        ...
        if (!f.exists()) {
            ...
            try (FileInputStream fis = new FileInputStream(f)) {
                ...
                for (int i = 1; i <= sheet.getLastRowNum(); i++) { // 循环每一行，从第3行开始（忽略表头）
                    ...
                    Cell cell8 = row.getCell(8); // 第0列
                    Cell cell9 = row.getCell(9); // 第34列


                    if (cell0.getStringCellValue().equals("桥梁名称") && dataFormatter.formatCellValue(cell8, formulaEvaluator).equals("总点数") ) { // 判断是否不为空
                        Cell nextRowCell8 = sheet.getRow(i + 1).getCell(8); // 下一行的第0列
                        Cell nextRowCell9 = sheet.getRow(i + 1).getCell(9); // 下一行的第34列
    ...
    public int selectnum(String proname, String htd) {
        int selectnum = jjgFbgcQlgcXbSzdMapper.selectnums(proname, htd);
        return selectnum;
    }

    @Override
    public int selectnumname(String proname) {
        int selectnum = jjgFbgcQlgcXbSzdMapper.selectnumname(proname);
        return selectnum;
    }

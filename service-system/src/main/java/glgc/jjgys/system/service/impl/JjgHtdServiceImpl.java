...
public class JjgHtdServiceImpl extends ServiceImpl<JjgHtdMapper, JjgHtd> implements JjgHtdService {
    ...
    @Override
    public void exporthtd(HttpServletResponse response, String workpath, String proname, String[] htd) {
        String zipname=proname+"合同段模板文件";
        JjgFbgcUtils.createDirectory(proname+"合同段模板文件",workpath);
        String filepath = workpath+"/"+zipname;

        if (htd.length>0){
            for (String h : htd) {
                String lxs=jjgHtdMapper.selectlx(proname,h);
                System.out.println("sssss"+lxs);
                if(lxs==null){
                    continue;
                }

                String[] lxlist=lxs.split(",");
                for(String lx:lxlist) {

                    System.out.println("测试"+htd);
                    JjgFbgcUtils.createDirectory(h, filepath);
                    String workpath1 = filepath + "/" + h;
                    switch (lx) {
                        case "路基工程":
                            jjgFbgcLjgcService.exportljgc(response, workpath1);
                            break;
                        case "路面工程":
                            jjgFbgcLmgcService.exportlmgc(response, workpath1);
                            break;
                        case "桥梁工程":
                            jjgFbgcQlgcService.exportqlgc(response, workpath1);
                            break;
                        case "隧道工程":
                            jjgFbgcSdgcService.exportsdgc(response, workpath1);
                            break;
                        case "交安工程":
                            jjgFbgcJagcService.exportjagc(response, workpath1);
                            break;
                    }
    ...
    @Override
    public void importhtd(MultipartFile file, CommonInfoVo commonInfoVo, String workpath) {
        ...
        File[] files=f_t.listFiles();

        for(File f:files){
            File[] files1=f.listFiles();
            String htd=f.getName();
            for(File f1:files1){
                ...
                switch (name){
                    case "路基工程":
                        jjgFbgcLjgcService.importljgc(commonInfoVo,path);
                        break;
                    case "路面工程":
                        jjgFbgcLmgcService.importlmgc(commonInfoVo,path);
                        break;
                    case "桥梁工程":
                        jjgFbgcQlgcService.importqlgc(commonInfoVo,path);
                        break;
                    case "隧道工程":
                        jjgFbgcSdgcService.importsdgc(commonInfoVo,path);
                        break;
                    case "交安工程":
                        jjgFbgcJagcService.importjagc(commonInfoVo,path);
                        break;
    ...
    @Override
    public void generateJdb(CommonInfoVo commonInfoVo, String[] htds) throws Exception {
        if (htds.length > 0) {
            for (String htd : htds) {
                commonInfoVo.setHtd(htd);
                String lxs = jjgHtdMapper.selectlx(commonInfoVo.getProname(), htd);
                if (lxs == null) {
                    continue;
                }
                String[] lxlist = lxs.split(",");
    ...
    @Override
    public void download(HttpServletResponse response, String filename, String proname, String workpath, String[] htds) {
        //List<Map<String, String>> htdlist = jjgHtdMapper.selecthtd1(proname);
       /* if (htds.length > 0) {
            for (String htd  : htds) {
                //String htd = map.get("name");
                workpath = workpath + "/" + htd;
                String[] lxs = htd.get("lx").split(",");

                for (String lx : lxs) {
                    switch (lx) {
                        case "路基工程":
                            jjgFbgcLjgcService.download(response, filename, workpath);
                            break;
                        case "路面工程":
                            jjgFbgcLmgcService.download(response, filename, proname, htd, workpath);
                            break;
                        case "桥梁工程":
                            jjgFbgcQlgcService.download(response, filename, proname, htd, workpath);
                            break;
                        case "隧道工程":
                            jjgFbgcSdgcService.download(response, filename, proname, htd, workpath);
                            break;
                        case "交安工程":
                            jjgFbgcJagcService.download(response, filename, proname, htd, workpath);
                            break;
                    }
                }
            }
    ...
    public boolean generateZip(String proname, String[] htds) {
        ...
        for (String htd : htds) {
            String selectlx = jjgHtdMapper.selectlx(proname, htd);
            String[] lxs = selectlx.split(",");
            for (String lx : lxs) {
                switch (lx) {
                    case "路基工程":
                        jjgFbgcLjgcService.getFile(workpath,htd);
                        break;
                    case "路面工程":
                        jjgFbgcLmgcService.getFile(workpath,proname, htd);
                        break;
                    case "桥梁工程":
                        jjgFbgcQlgcService.getFile(workpath,proname, htd);
                        break;
                    case "隧道工程":
                        jjgFbgcSdgcService.getFile(workpath,proname, htd);
                        break;
                    case "交安工程":
                        jjgFbgcJagcService.getFile(workpath,proname, htd);
                        break;
                }
            }
        }
